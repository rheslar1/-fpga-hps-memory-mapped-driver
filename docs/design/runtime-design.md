...
1. Build a runtime profile for `fpga-hps-memory-mapped-driver`.
...
The runtime must make this focus measurable through logs, reports, tests, diagrams, or hardware captures. The proof point for review is: Hardware co-processing, memory-mapped register control, FPGA/Linux integration, and low-level driver reasoning.
